...
- In the RHOAI Dashboard, navigate in the left panel to Settings > Runtimes (In RHOAI 3., navigate Settings > Model resources and operations > Serving runtimes). Then, 
- Click "Add serving runtime" button. 
- In the editor, copy the contents from the `llm-cpu-custom-runtime.yaml` file in [vllm-cpu-custom-runtime.yaml](https://github.com/alexbarbosa1989/ai-playground/blob/main/rhoai2-cpu-tests/vllm-cpu-custom-runtime.yaml). It references the `quay.io/abarbosa/vllm-cpu:0.22.1` image built for these test purposes.
